##  Vylepši riešenie (5) úlohy:
##  pri kreslení polygónu sa automaticky všetky '+' z plochy vymažú
##  (canvas.delete(...)).
##  Každému nakreslenému '+' môžeš pridať rovnaký tag a potom ich naraz zrušíš.
##  Pozri, napríklad 3. prednášku.


#úvodní a prázdný řádek:
print('BAREVNÉ POLYHGONY BEZ PLUSŮ')
...
na místě vaznačeném body vytvoří polygon vyplněný náhodnou barvou, smaže '+'
...
def klik(event):
    ...
    canvas.create_text(x, y, text='+', tag='+')             #příkaz na nakreslení znaku "+"
    ...
    if poc > 2:                                             #podmínka - pokud počítadlo má minimálně hodnotu 2 (jede se třetí kolo)
        if abs(sum(sez[0])-(x+y)) < 5:                      #podmínka - pokud je aktuální souřadnice na 5 bodů od první souřadnice
            ...
            canvas.delete('+')                              #příkaz ke smazání všech objektů s tagem '+'
